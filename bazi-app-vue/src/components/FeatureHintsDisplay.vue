<template>
  <div class="feature-hints-display">
    ...
    <!-- Backdrop for mobile -->
    <div v-if="showTooltip" class="tooltip-backdrop" @click="closeTooltip" />
  </div>
</template>
...
const minorStarHints = computed(() => {
  ...
  for (const [category, starNames] of Object.entries(categories)) {
    const hasCategory = minorStars.some((star) =>
      starNames.includes(star.name),
    );
    if (hasCategory) {
      hints.push({
        category: category.toLowerCase(),
        description: `${category}雜曜`,
      });
    }
  }
  ...
  switch (type) {
    ...
    case 'empty':
      tooltipData.value = {
        ...
        advice: hint.borrowedInfo
          ? `可借用${hint.borrowedInfo.name}的星曜特質。`
          : '需要主動開創該領域的運勢。',
      };
      break;
    case 'minor':
...
const getPatternDescription = (pattern: string) => {
  ...
  return (
    descriptions[pattern as keyof typeof descriptions] ||
    '特殊星曜組合，具有獨特影響力。'
  );
};
...
const getPatternInfluence = (pattern: string) => {
  ...
  return (
    influences[pattern as keyof typeof influences] || '帶來正面的綜合影響。'
  );
};
...
const getMinorStarDescription = (category: string) => {
  ...
  return (
    descriptions[category as keyof typeof descriptions] || '特殊類型的雜曜。'
  );
};
...
const getMinorStarInfluence = (category: string) => {
  ...
  return (
    influences[category as keyof typeof influences] || '帶來特殊的影響力。'
  );
};
