...
export class GeocodeService {
    static BASE_URL = 'https://geocode.arcgis.com/arcgis/rest/services/World/GeocodeServer/findAddressCandidates';
    static async geocodeAddress(address) {
        try {
            ...
            const validCandidates = response.data.candidates.filter(candidate => candidate.score >= 60);
            return {
                success: true,
                candidates: validCandidates,
                error: validCandidates.length === 0 ? '找不到匹配的地址，請檢查地址格式' : undefined
            };
        }
        catch (error) {
