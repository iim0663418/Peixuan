...
export class BaziCalculator {
    static calculateBazi(input) {
        try {
            ...
            const yearStem = lunarDate.getYearGan();
            const yearBranch = lunarDate.getYearZhi();
            const monthStem = lunarDate.getMonthGan();
            const monthBranch = lunarDate.getMonthZhi();
            const dayStem = lunarDate.getDayGan();
            const dayBranch = lunarDate.getDayZhi();
            const timeGanZhi = lunarDate.getTimeInGanZhi();
            const hourStem = timeGanZhi.substring(0, 1);
            const hourBranch = timeGanZhi.substring(1, 2);
            return {
...
export class TenGodsCalculator {
    static getTenGodRelation(dayMaster, otherStem) {
    ...
    static getMainStemTenGods(baziResult) {
...
export class FiveElementsAnalyzer {
    static calculateElementsDistribution(baziResult) {
...
export class FortuneCycleCalculator {
     ...
     * @returns 起運年齡和年份
     */
    static calculateStartLuck(lunarDate, gender) {
        try {
        ...
        try {
            ...
            for (let i = 0; i < count; i++) {
                ...
                if (isForward) {
                    ...
                    // 逆行
                    stemIndex = (monthStemIndex - i + HEAVENLY_STEMS.length) % HEAVENLY_STEMS.length;
                    branchIndex = (monthBranchIndex - i + EARTHLY_BRANCHES.length) % EARTHLY_BRANCHES.length;
                }
     ...
     * @returns 流年資料陣列
     */
    static calculateAnnualLuck(birthDate, startYear, years = 30) {
        try {
...
// 命盤解讀層次
export var InterpretationLevel;
(function (InterpretationLevel) {
    InterpretationLevel["BASIC"] = "basic";
    InterpretationLevel["INTERMEDIATE"] = "intermediate";
    InterpretationLevel["EXPERT"] = "expert"; // 專家解讀
...
export class BaziInterpreter {
     ...
     * @returns 基礎命盤解讀
     */
    static generateBasicInterpretation(baziResult) {
        const dayMaster = baziResult.dayPillar.stem;
        const dayMasterElement = STEM_TO_ELEMENT[dayMaster];
        // 計算五行分佈
        const elementsDist = FiveElementsAnalyzer.calculateElementsDistribution(baziResult);
        // 確定旺弱
        const strongElements = [];
        const weakElements = [];
        // 簡單實現：根據數量判斷五行旺弱
        const elements = Object.entries(elementsDist);
        elements.sort((a, b) => b[1] - a[1]);
        // 最多的兩個五行視為旺
        strongElements.push(elements[0][0], elements[1][0]);
        // 最少的兩個五行視為弱
        weakElements.push(elements[elements.length - 1][0], elements[elements.length - 2][0]);
        // 判斷日主喜忌
        ...
        // 產生性格特質
        const personalityTraits = this.generatePersonalityTraits(baziResult, dayMasterElement);
        // 產生重要年齡
     ...
     * 基於八字生成性格特質描述
     */
    static generatePersonalityTraits(baziResult, dayMasterElement) {
        const traits = [];
        // 基於日主五行的基本性格
        switch (dayMasterElement) {
            case '木':
                traits.push('性格正直開朗，具有理想和抱負', '做事有計劃性，善於組織和規劃', '有同情心和公正感');
                break;
            case '火':
                traits.push('熱情洋溢，充滿活力', '善於表達，人緣良好', '直覺敏銳，創造力強');
                break;
            case '土':
                traits.push('踏實穩重，忠誠可靠', '意志堅定，做事有耐心', '思慮周全，處事圓融');
                break;
            case '金':
                traits.push('做事有條理，處事公正', '意志堅強，自律性高', '善於分析，判斷力強');
                break;
            case '水':
                traits.push('思維靈活，適應力強', '善於溝通，洞察力敏銳', '好奇心強，求知欲旺盛');
                break;
        }
        // 基於年柱的性格影響（童年及與父母關係）
     ...
     * 生成人生模式描述
     */
    static generateLifePatternDescription(baziResult) {
     ...
     * 生成重要年齡點
     */
    static generateKeyAges(baziResult) {
     ...
     * 生成事業建議
     */
    static generateCareerAdvice(baziResult, favorableElements) {
        let advice = '從職業選擇來看，';
     ...
     * 生成人際關係建議
     */
    static generateRelationshipAdvice(baziResult) {
     ...
     * 生成健康建議
     */
    static generateHealthAdvice(baziResult, dayMasterElement) {
        // 基於日主五行的健康建議
        switch (dayMasterElement) {
            case '木':
     ...
     * @returns 流年解讀
     */
    static generateAnnualAnalysis(baziResult, annualLuck, level = InterpretationLevel.BASIC) {
        const dayMaster = baziResult.dayPillar.stem;
     ...
     * @returns 大運解讀
     */
    static generateDecennialAnalysis(baziResult, decennialCycle, level = InterpretationLevel.BASIC) {
        const dayMaster = baziResult.dayPillar.stem;
        const decennialStem = decennialCycle.stem;
        const decennialBranch = decennialCycle.branch;
        // 取得天干關係
        const tenGod = TenGodsCalculator.getTenGodRelation(dayMaster, decennialStem);
        // 大運概述
        ...
        switch (tenGod) {
            ...
            case '劫財':
                ...
                relationships = '人際關係活躍，可能結交志同道合的朋友，但需處理好競爭關係。';
            ...
            case '七殺':
            case '正官':
                career = '事業上可能面臨挑戰但也有突破，適合從事管理、法律或權威性工作。';
            ...
            case '偏印':
            case '正印':
                career = '適合學術研究、教育或需要專業知識的工作，有機會深造或考取證書。';
...
//# sourceMappingURL=baziCalc.js.map