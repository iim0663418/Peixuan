...
export class GeocodeService {
  private static readonly BASE_URL = 'https://geocode.arcgis.com/arcgis/rest/services/World/GeocodeServer/findAddressCandidates';
  ...
  private static getAccuracyText(score: number): string {
    if (score >= 95) return '準確';
    if (score >= 80) return '良好';
    if (score >= 60) return '模糊';
    return '不準確';
  }

  static formatCoordinates(longitude: number, latitude: number): { longitude: number; latitude: number } {
    return {
