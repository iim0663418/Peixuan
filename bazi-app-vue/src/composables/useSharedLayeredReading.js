...
// 全域共享狀態
const purpleStarReadingLevel = ref(ReadingLevel.STANDARD);
const isInitialized = ref(false);
/**
 * 紫微斗數和四化飛星共享的分層閱讀管理
 * 紫微斗數為主控，四化飛星自動同步
 */
export function useSharedLayeredReading(moduleType) {
    const { readingState, userPreferences, currentBreakpoint, availableLevels, currentLevelConfig, canUpgrade, canDowngrade, switchToLevel, upgradeLevel, downgradeLevel, autoSelectOptimalLevel, updateLayeredData, saveUserPreferences, resetToDefaults } = useLayeredReading();
    // 初始化共享狀態
    if (!isInitialized.value) {
        // 從 sessionStorage 初始化或使用預設值
        const savedLevel = sessionStorage.getItem('purple-star-reading-level');
    ...
    // 監聽紫微斗數層級變化（僅四化飛星需要）
    if (moduleType === 'transformationStars') {
        const handlePurpleStarLevelChange = (event) => {
        ...
        window.addEventListener('purple-star-level-changed', handlePurpleStarLevelChange);
    ...
    // 同步方法包裝
    const syncedSwitchToLevel = async (level) => {
 ...
 * 取得當前紫微斗數的閱讀層級（供外部查詢）
 */
export function getPurpleStarReadingLevel() {
    return purpleStarReadingLevel.value;
}
/**
 * 重置所有共享狀態
...
//# sourceMappingURL=useSharedLayeredReading.js.map