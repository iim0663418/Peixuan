...
class PurpleStarCalculator {
    ...
    shenPalaceStdIndex;
    fiveElementsBureau;
    fiveElementsBureauNumber;
    mingPalaceGan;
    ZHI_NAMES = ['子', '丑', '寅', '卯', '辰', '巳', '午', '未', '申', '酉', '戌', '亥'];
    GAN_NAMES = ['甲', '乙', '丙', '丁', '戊', '己', '庚', '辛', '壬', '癸'];
    PALACE_NAMES_ORDERED = [
        '命宮', '兄弟宮', '夫妻宮', '子女宮', '財帛宮', '疾厄宮',
        '遷移宮', '交友宮', '官祿宮', '田宅宮', '福德宮', '父母宮',
    ];
    ZIWEI_STAR_SYSTEM = ['紫微', '天機', '太陽', '武曲', '天同', '廉貞'];
    TIANFU_STAR_SYSTEM = ['天府', '太陰', '貪狼', '巨門', '天相', '天梁', '七殺', '破軍'];
    ...
    getChineseHourZhiIndex() { return this.ZHI_NAMES.indexOf(this.lunarDate.getTimeZhi()); }
    calculateMingAndShenPalaceIndices() {
    ...
    getMingPalaceGan() {
        ...
        const ganIndex = (gan) => this.GAN_NAMES.indexOf(gan);
    ...
    calculateFiveElementsBureau() {
        ...
        if (["甲子", "乙丑", "丙子", "丁丑", "甲寅", "乙卯", "甲申", "乙酉", "壬子", "癸丑", "壬戌", "癸亥"].includes(mingGanZhi))
            return "水二局";
        if (["戊辰", "己巳", "壬午", "癸未", "戊戌", "己亥", "壬申", "癸酉", "庚寅", "辛卯", "庚申", "辛酉"].includes(mingGanZhi))
            return "木三局";
        if (["甲午", "乙未", "壬寅", "癸卯", "庚辰", "辛巳", "庚戌", "辛亥", "甲辰", "乙巳", "丙申", "丁酉"].includes(mingGanZhi))
            return "金四局";
        if (["戊寅", "己卯", "丙戌", "丁亥", "庚午", "辛未", "戊申", "己酉", "庚子", "辛丑", "丙辰", "丁巳"].includes(mingGanZhi))
            return "土五局";
        if (["丙寅", "丁卯", "甲戌", "乙亥", "戊子", "己丑", "戊午", "己未", "丙午", "丁未", "壬辰", "癸巳"].includes(mingGanZhi))
            return "火六局";
    ...
    calculatePalaces(mingPalaceStdIndex) {
        const palaces = [];
        for (let i = 0; i < 12; i++) {
    ...
    placeStar(palaces, starName, starType, palaceIndex) {
        const star = { name: starName, type: starType, palaceIndex, transformations: [] };
    ...
    calculateMainStars(palaces, ziweiPalaceStdIndex) {
        const allMainStars = [];
        ...
        const tianfuLocationMap = { [2]: 2, [8]: 8, [1]: 3, [3]: 1, [0]: 4, [4]: 0, [11]: 5, [5]: 11, [10]: 6, [6]: 10, [9]: 7, [7]: 9 };
    ...
    calculateAuxiliaryStars(palaces) {
        ...
        const tianKongPalaceIndex = (HAI_PALACE_STD_INDEX - chineseHourZhiIndex + 12) % 12;
    ...
    calculateDaXian(palaces, maxAge = 120) {
        const daXianList = [];
     ...
     */
    calculateXiaoXian(palaces, maxAge = 100) {
        const xiaoXianList = [];
        const yearZhi = this.lunarDate.getYearZhi(); // 出生年地支
        const isMale = this.birthInfo.gender === 'male';
        let startPalaceZhiIndex;
        // 口訣：寅午戌局從辰起；申子辰局從戌起；巳酉丑局從未起；亥卯未局從丑起。
        // 地支索引: 子0丑1寅2卯3辰4巳5午6未7申8酉9戌10亥11
        if (['寅', '午', '戌'].includes(yearZhi))
            startPalaceZhiIndex = this.ZHI_NAMES.indexOf('辰'); // 辰=4
        else if (['申', '子', '辰'].includes(yearZhi))
            startPalaceZhiIndex = this.ZHI_NAMES.indexOf('戌'); // 戌=10
        else if (['巳', '酉', '丑'].includes(yearZhi))
            startPalaceZhiIndex = this.ZHI_NAMES.indexOf('未'); // 未=7
        else if (['亥', '卯', '未'].includes(yearZhi))
            startPalaceZhiIndex = this.ZHI_NAMES.indexOf('丑'); // 丑=1
        else
            throw new Error(`無法識別的年支用於小限起點: ${yearZhi}`);
        for (let age = 1; age <= maxAge; age++) {
            let currentPalaceZhiIndex;
            if (isMale) { // 男順行
    ...
    // TODO: 實作流年太歲的計算
    determinePalaceAttributes(palaces) { }
    assembleChart(palaces) {
...
*/
//# sourceMappingURL=ziweiCalc.js.map