    ...
    <div class="fortune-container">
      ...
      <div class="overall-fortune-section">
        <div class="score-display">
          ...
          <div class="score-info">
            <div class="grade-title">{{ getGradeTitle(overallFortune.grade) }}</div>
      ...
      <div class="palace-fortune-section">
        ...
        <div class="palace-grid">
          ...
          >
            <div class="palace-name">{{ palace.palaceName.replace('宮', '') }}</div>
            <div class="fortune-indicator" :class="[`indicator-${palace.status}`]">
        ...
        <div class="advice-cards">
          ...
          >
            <div class="advice-header">
              <span class="priority-badge">{{ getPriorityText(advice.priority) }}</span>
...
const starFortuneScores: Record<string, number> = {
  // 主星
  '紫微': 8, '天機': 6, '太陽': 7, '武曲': 7, '天同': 6, '廉貞': 5,
  '天府': 8, '太陰': 6, '貪狼': 5, '巨門': 4, '天相': 7, '天梁': 7,
  '七殺': 4, '破軍': 3,
  
  // 吉星
  '文昌': 3, '文曲': 3, '左輔': 4, '右弼': 4, '天魁': 3, '天鉞': 3,
  '祿存': 5, '天馬': 2, '紅鸞': 2, '天喜': 2, '化祿': 5, '化權': 4, '化科': 3,
  
  // 凶星
  '擎羊': -3, '陀羅': -3, '火星': -2, '鈴星': -2, '天空': -2, '地劫': -2,
  '化忌': -4
}

// 宮位重要性權重
const palaceWeights: Record<string, number> = {
  '命宮': 1.0, '財帛宮': 0.9, '官祿宮': 0.9, '夫妻宮': 0.8,
  '遷移宮': 0.7, '疾厄宮': 0.7, '田宅宮': 0.6, '福德宮': 0.6,
  '父母宮': 0.5, '子女宮': 0.5, '兄弟宮': 0.4, '奴僕宮': 0.4
}
    ...
    palace.stars.forEach(star => {
      ...
      if (star.brightness) {
        const brightnessScores: Record<string, number> = {
          '廟': 1.3, '旺': 1.2, '得地': 1.1, '利益': 1.0, '平和': 0.9, '不得地': 0.8, '落陷': 0.7
        }
...
const palaceFortuneStatuses = computed(() => {
  ...
  return props.chartData.palaces.map(palace => {
    ...
    palace.stars.forEach(star => {
      ...
      if (star.brightness) {
        const multipliers: Record<string, number> = {
          '廟': 1.3, '旺': 1.2, '得地': 1.1, '平和': 0.9, '不得地': 0.8, '落陷': 0.7
        }
    ...
    else if (finalScore >= 45) status = 'average'
    else if (finalScore >= 30) status = 'challenging'
...
const importantStarTips = computed(() => {
  ...
  if (!props.chartData?.palaces) return tips
...
const actionAdvice = computed(() => {
  ...
  const score = overallFortune.value.score
...
const getTransformationDescription = (transformation: string, palaceName: string): string => {
...
.score-excellent { border-color: #28a745; background: rgba(40, 167, 69, 0.1); }
.score-good { border-color: #17a2b8; background: rgba(23, 162, 184, 0.1); }
.score-average { border-color: #ffc107; background: rgba(255, 193, 7, 0.1); }
.score-challenging { border-color: #fd7e14; background: rgba(253, 126, 20, 0.1); }
.score-difficult { border-color: #dc3545; background: rgba(220, 53, 69, 0.1); }
...
.fortune-excellent { border-color: #28a745; }
.fortune-good { border-color: #17a2b8; }
.fortune-average { border-color: #ffc107; }
.fortune-challenging { border-color: #fd7e14; }
.fortune-difficult { border-color: #dc3545; }
...
.indicator-excellent { background: #28a745; color: white; }
.indicator-good { background: #17a2b8; color: white; }
.indicator-average { background: #ffc107; color: #212529; }
.indicator-challenging { background: #fd7e14; color: white; }
.indicator-difficult { background: #dc3545; color: white; }
...
.highlight-opportunity { border-left-color: #28a745; }
.highlight-caution { border-left-color: #ffc107; }
.highlight-harvest { border-left-color: #fd7e14; }
.highlight-preparation { border-left-color: #6f42c1; }
