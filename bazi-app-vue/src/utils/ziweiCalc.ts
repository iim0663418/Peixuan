...
export interface Palace { // 新增 export
...
export interface DaXianInfo { // 新增 export
  startAge: number;
  endAge: number;
  palaceName: string;
  palaceZhi: string;
...
export interface XiaoXianInfo { // 新增 export
  age: number; // 虛歲
  palaceName: string;
  palaceZhi: string;
  palaceIndex: number;
...
export interface LiuNianTaiSuiInfo { // 新增 export
  year: number; // 公曆年
  ganZhi: string; // 流年干支
  palaceName: string;
  palaceZhi: string;
...
export interface PurpleStarChart { // 新增 export
  palaces: Palace[];
...
export interface BirthInfo { // 新增 export
...
class PurpleStarCalculator {
  ...
  constructor(birthInfo: BirthInfo) {
    ...
    const solarInstance = Solar.fromYmdHms(this.solarDate.getFullYear(), this.solarDate.getMonth() + 1, this.solarDate.getDate(), this.solarDate.getHours(), this.solarDate.getMinutes(), this.solarDate.getSeconds());
    this.lunarDate = solarInstance.getLunar();

    console.log('Converted Lunar Date:', {
      year: this.lunarDate.getYear(), month: this.lunarDate.getMonth(), day: this.lunarDate.getDay(),
      hour: this.lunarDate.getHour(), yearGanzhi: this.lunarDate.getYearInGanZhi(),
      monthGanzhi: this.lunarDate.getMonthInGanZhi(), dayGanzhi: this.lunarDate.getDayInGanZhi(),
      timeGanzhi: this.lunarDate.getTimeInGanZhi(),
    });
  ...
  private getChineseHourZhiIndex(): number { return this.ZHI_NAMES.indexOf(this.lunarDate.getTimeZhi()); }
  private calculateMingAndShenPalaceIndices(): { mingPalaceStdIndex: number; shenPalaceStdIndex: number } {
    const YIN_PALACE_STD_INDEX = 2; const lunarMonth = this.lunarDate.getMonth(); const chineseHourZhiIndex = this.getChineseHourZhiIndex();
    let mingPalaceStdIndex = (YIN_PALACE_STD_INDEX + (lunarMonth - 1) - chineseHourZhiIndex); mingPalaceStdIndex = (mingPalaceStdIndex % 12 + 12) % 12;
  ...
  private getMingPalaceGan(): string {
    const yearGan = this.lunarDate.getYearGan(); let yinPalaceGan: string;
    if (['甲', '己'].includes(yearGan)) yinPalaceGan = '丙'; else if (['乙', '庚'].includes(yearGan)) yinPalaceGan = '戊';
    else if (['丙', '辛'].includes(yearGan)) yinPalaceGan = '庚'; else if (['丁', '壬'].includes(yearGan)) yinPalaceGan = '壬';
    else if (['戊', '癸'].includes(yearGan)) yinPalaceGan = '甲'; else throw new Error('Invalid year Gan');
    const ganIndex = (gan: string) => this.GAN_NAMES.indexOf(gan); const yinPalaceGanIndex = ganIndex(yinPalaceGan);
    const mingPalaceZhiIndex = this.mingPalaceStdIndex; const yinPalaceStdIndex = 2;
    let diff = (mingPalaceZhiIndex - yinPalaceStdIndex + 12) % 12; const mingPalaceGanIndex = (yinPalaceGanIndex + diff) % 10;
  ...
  private locateZiWeiStar(): number {
    const lunarDay = this.lunarDate.getDay(); const bureauNum = this.fiveElementsBureauNumber; if (bureauNum === undefined) throw new Error("五行局數字未定義");
    let quotient = Math.floor(lunarDay / bureauNum); let remainder = lunarDay % bureauNum; let n = 0;
    if (remainder !== 0) { n = bureauNum - remainder; quotient = Math.floor((lunarDay + n) / bureauNum); }
    const YIN_PALACE_STD_INDEX = 2; let ziweiPalaceStdIndex = (YIN_PALACE_STD_INDEX + quotient - 1 + 12) % 12;
    if (n !== 0) { if (n % 2 === 1) ziweiPalaceStdIndex = (ziweiPalaceStdIndex - 1 + 12) % 12; else ziweiPalaceStdIndex = (ziweiPalaceStdIndex + 1) % 12; }
  ...
  private calculatePalaces(mingPalaceStdIndex: number): Palace[] {
    ...
    for (let i = 0; i < 12; i++) {
      ...
      palaces.push({ name: this.PALACE_NAMES_ORDERED[i], index: currentPalaceStdZhiIndex, zhi: this.ZHI_NAMES[currentPalaceStdZhiIndex], stars: [] });
    }
    return palaces;
  }
  private placeStar(palaces: Palace[], starName: string, starType: 'main' | 'auxiliary' | 'minor', palaceIndex: number): Star {
    const star: Star = { name: starName, type: starType, palaceIndex, transformations: [] };
    const palace = palaces.find(p => p.index === palaceIndex);
    if (palace) palace.stars.push(star); else console.error(`無法找到地支索引為 ${palaceIndex} 的宮位來安放 ${starName} 星`);
    return star;
  }
  private calculateMainStars(palaces: Palace[], ziweiPalaceStdIndex: number): Star[] {
    const allMainStars: Star[] = []; const ziweiSystemOffsets = [0, -1, -3, -4, -5, -8];
    for (let i = 0; i < this.ZIWEI_STAR_SYSTEM.length; i++) {
      const starName = this.ZIWEI_STAR_SYSTEM[i]; const palaceIndex = (ziweiPalaceStdIndex + ziweiSystemOffsets[i] + 12) % 12;
      allMainStars.push(this.placeStar(palaces, starName, 'main', palaceIndex));
    }
    const tianfuLocationMap: Record<number, number> = { [2]:2, [8]:8, [1]:3, [3]:1, [0]:4, [4]:0, [11]:5, [5]:11, [10]:6, [6]:10, [9]:7, [7]:9 };
    const tianfuPalaceStdIndex = tianfuLocationMap[ziweiPalaceStdIndex]; if (tianfuPalaceStdIndex === undefined) throw new Error("無法定位天府星");
    const tianfuSystemOffsets = [0, 1, 2, 3, 4, 5, 6, 10];
    for (let i = 0; i < this.TIANFU_STAR_SYSTEM.length; i++) {
      const starName = this.TIANFU_STAR_SYSTEM[i]; const palaceIndex = (tianfuPalaceStdIndex + tianfuSystemOffsets[i]) % 12;
      allMainStars.push(this.placeStar(palaces, starName, 'main', palaceIndex));
    }
    return allMainStars;
  }
  private calculateAuxiliaryStars(palaces: Palace[]): Star[] {
    const auxiliaryStars: Star[] = []; const lunarMonth = this.lunarDate.getMonth(); const chineseHourZhiIndex = this.getChineseHourZhiIndex();
    const CHEN_PALACE_STD_INDEX = 4; const zuoFuPalaceIndex = (CHEN_PALACE_STD_INDEX + (lunarMonth - 1) + 12) % 12;
    auxiliaryStars.push(this.placeStar(palaces, '左輔', 'auxiliary', zuoFuPalaceIndex));
    const XU_PALACE_STD_INDEX = 10; const youBiPalaceIndex = (XU_PALACE_STD_INDEX - (lunarMonth - 1) + 12) % 12;
    auxiliaryStars.push(this.placeStar(palaces, '右弼', 'auxiliary', youBiPalaceIndex));
    const wenChangPalaceIndex = (XU_PALACE_STD_INDEX - chineseHourZhiIndex + 12) % 12;
    auxiliaryStars.push(this.placeStar(palaces, '文昌', 'auxiliary', wenChangPalaceIndex));
    const wenQuPalaceIndex = (CHEN_PALACE_STD_INDEX + chineseHourZhiIndex + 12) % 12;
    auxiliaryStars.push(this.placeStar(palaces, '文曲', 'auxiliary', wenQuPalaceIndex));
    const HAI_PALACE_STD_INDEX = 11; const tianKongPalaceIndex = (HAI_PALACE_STD_INDEX - chineseHourZhiIndex + 12) % 12;
    auxiliaryStars.push(this.placeStar(palaces, '天空', 'auxiliary', tianKongPalaceIndex));
    const diJiePalaceIndex = (HAI_PALACE_STD_INDEX + chineseHourZhiIndex + 12) % 12;
  ...
  private applyFourTransformations(palaces: Palace[], targetGan: string): void {
    const transformations = this.FOUR_TRANSFORMATIONS_MAP[targetGan]; if (!transformations) { console.warn(`找不到天干 ${targetGan} 的四化表`); return; }
    const applyTransform = (starName: string, transformType: TransformationType) => {
      palaces.forEach(palace => { palace.stars.forEach(star => {
          if (star.name === starName) { if (!star.transformations) star.transformations = []; star.transformations.push(transformType); }
      }); });
    };
    applyTransform(transformations.lu, '祿'); applyTransform(transformations.quan, '權');
    applyTransform(transformations.ke, '科'); applyTransform(transformations.ji, '忌');
  }
  ...
  public calculateDaXian(palaces: Palace[], maxAge: number = 120): DaXianInfo[] {
    const daXianList: DaXianInfo[] = [];
    if (this.fiveElementsBureauNumber === undefined) throw new Error("五行局數字未定義，無法計算大限");
    ...
    while (currentAge <= maxAge) {
      ...
      if (!daXianPalace) { console.error(`無法找到大限宮位: order index ${daXianPalaceOrderIndex}`); break; }
      daXianList.push({
        startAge: currentAge, endAge: currentAge + 9,
        palaceName: daXianPalace.name, palaceZhi: this.ZHI_NAMES[daXianPalace.index],
   ...
   */
  public calculateXiaoXian(palaces: Palace[], maxAge: number = 100): XiaoXianInfo[] {
    ...
    if (['寅', '午', '戌'].includes(yearZhi)) startPalaceZhiIndex = this.ZHI_NAMES.indexOf('辰'); // 辰=4
    else if (['申', '子', '辰'].includes(yearZhi)) startPalaceZhiIndex = this.ZHI_NAMES.indexOf('戌'); // 戌=10
    else if (['巳', '酉', '丑'].includes(yearZhi)) startPalaceZhiIndex = this.ZHI_NAMES.indexOf('未'); // 未=7
    else if (['亥', '卯', '未'].includes(yearZhi)) startPalaceZhiIndex = this.ZHI_NAMES.indexOf('丑'); // 丑=1
    ...
    console.log('Calculated XiaoXian (up to age 10):', xiaoXianList.slice(0,10));
    return xiaoXianList;
  }
  ...
  public generateChart(): PurpleStarChart {
    ...
    console.log('Generated Purple Star Chart:', chart);
    return chart;
  }
}
