...
describe('EnhancedStorageService', () => {
  ...
  beforeEach(() => {
    ...
    // 模擬基本存儲服務方法
    vi.spyOn(storageService, 'saveToStorage').mockImplementation(
      (key, value) => {
  ...
  it('應該正確初始化並設置存儲版本', () => {
    ...
    const version = storageService.getFromStorage(
      ENHANCED_STORAGE_KEYS.STORAGE_VERSION,
    );
  ...
  it('應該能夠保存和獲取用戶偏好設定', () => {
    ...
    // 驗證時區資訊也被單獨保存
    const timeZoneInfo = storageService.getFromStorage<{
      timeZone: string;
      timestamp: number;
    }>(ENHANCED_STORAGE_KEYS.TIMEZONE_INFO);
  ...
  it('應該能夠驗證存儲資料的完整性', () => {
    ...
    // 檢查是否更新了最後驗證時間
    const lastValidated = storageService.getFromStorage(
      ENHANCED_STORAGE_KEYS.STORAGE_LAST_VALIDATED,
    );
  ...
  it('應該能夠進行資料遷移', () => {
    ...
    storageService.saveToStorage(
      ENHANCED_STORAGE_KEYS.BAZI_BIRTH_INFO,
      oldBirthInfo,
    );
    ...
    // 驗證版本已更新
    const newVersion = storageService.getFromStorage(
      ENHANCED_STORAGE_KEYS.STORAGE_VERSION,
    );
