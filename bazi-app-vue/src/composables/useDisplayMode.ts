...
export function useDisplayMode(componentName: string) {
  ...
  const loadSavedMode = () => {
    try {
      const savedMode = sessionStorage.getItem(
        `${componentName}-display-depth`,
      );
      if (
        savedMode &&
        ['minimal', 'compact', 'standard', 'comprehensive'].includes(savedMode)
      ) {
        globalModes.value[componentName] = savedMode as DisplayMode;
      }
    } catch (e) {
      console.error(
        `無法從 sessionStorage 載入 ${componentName} 的顯示深度設定:`,
        e,
      );
    }
  };
  ...
  const handleDisplayDepthChanged = (event: CustomEvent) => {
    if (
      event.detail &&
      event.detail.module === componentName &&
      event.detail.depth !== globalModes.value[componentName]
    ) {
  ...
  onMounted(() => {
    window.addEventListener(
      'display-depth-changed',
      handleDisplayDepthChanged as EventListener,
    );
    window.addEventListener(
      'module-changed',
      handleModuleChanged as EventListener,
    );
  });
