...
export function splitIntoSentences(text: string): string[] {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    for (let j = 0; j < line.length; j++) {
      ...
      const isEnglishEnd = ['.', '!', '?', ';'].includes(char) &&
      ...
      if (isChineseEnd || isEnglishEnd) {
        ...
        const keepTogether = ['」', '』', '）', ')', '"', "'"].includes(nextChar)
 ...
 */
export function calculateDelay(sentenceIndex: number, sentenceLength: number): number {
 ...
 */
export function chunkSentences(sentences: string[], maxChunkSize: number = 3): string[] {
  const chunks: string[] = []
  let currentChunk: string[] = []

  for (const sentence of sentences) {
    // Headers and lists should be in their own bubble
    if (sentence.match(/^#{1,6}\s+/) || sentence.match(/^[-*+]\s+/) || sentence.match(/^\d+\.\s+/)) {
