...
describe('LanguageSelector', () => {
    ...
    it('handles localStorage errors gracefully', () => {
        ...
        // 應該記錄警告並使用預設語言
        expect(consoleWarnSpy).toHaveBeenCalledWith('Failed to load language preference from localStorage:', expect.any(Error));
        expect(i18n.global.locale.value).toBe('en');
    ...
    it('handles save errors gracefully', async () => {
        ...
        await select.trigger('change');
        // 應該記錄警告但不影響語言切換
        expect(consoleWarnSpy).toHaveBeenCalledWith('Failed to save language preference to localStorage:', expect.any(Error));
        expect(i18n.global.locale.value).toBe('zh_TW');
        consoleWarnSpy.mockRestore();
    });
...
//# sourceMappingURL=LanguageSelector.spec.js.map