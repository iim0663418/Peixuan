 ...
 */
export function useDisplayMode(componentName) {
    // 內部更新標誌，防止循環觸發
    ...
    const loadSavedMode = () => {
        try {
            ...
            if (savedMode && ['minimal', 'compact', 'standard', 'comprehensive'].includes(savedMode)) {
                globalModes.value[componentName] = savedMode;
            }
        }
        catch (e) {
    ...
    loadSavedMode();
    // 監聽顯示深度變更事件
    const handleDisplayDepthChanged = (event) => {
    ...
    const handleModuleChanged = (event) => {
        if (event.detail && event.detail.module === componentName && event.detail.depth !== globalModes.value[componentName]) {
            isUpdatingFromEvent = true;
            globalModes.value[componentName] = event.detail.depth;
            // 同步保存到 sessionStorage
    ...
    onMounted(() => {
        window.addEventListener('display-depth-changed', handleDisplayDepthChanged);
        window.addEventListener('module-changed', handleModuleChanged);
    });
    onBeforeUnmount(() => {
        window.removeEventListener('display-depth-changed', handleDisplayDepthChanged);
        window.removeEventListener('module-changed', handleModuleChanged);
    });
    return {
        displayMode,
        mapDepthToMode,
...
//# sourceMappingURL=useDisplayMode.js.map