    ...
    <div class="overview-container">
      ...
      <div class="talents-traits-section">
        ...
        <div class="cards-grid">
          ...
          >
            ...
            <div class="card-content">
              ...
              <div class="star-config" v-if="talent.keyStars.length > 0">
        ...
        <div v-if="personalTalents.length === 0" class="no-insights">
          ...
          <p>命盤呈現均衡發展的特質，各方面都有發展潛力，建議多元嘗試發掘個人興趣。</p>
        </div>
      </div>
        ...
        <div class="cards-grid">
          ...
          >
            ...
            <div class="card-content">
              ...
              <div class="development-method" v-if="potential.developmentMethod">
            ...
            <div class="energy-distribution">
              ...
              <div class="energy-bars">
                ...
                >
                  ...
                  <span class="energy-value">{{ energy > 0 ? '+' : '' }}{{ energy }}</span>
                </div>
              </div>
            </div>
          </div>
      ...
      <!-- 關鍵格局 -->
      <div class="key-patterns-section" v-if="keyPatterns.length > 0">
        <h4 class="section-title">
          <span class="section-icon">🔮</span>
          關鍵格局分析
        </h4>
...
const personalTalents = computed(() => {
  ...
  if (!props.chartData?.palaces) return talents
  ...
  props.chartData.palaces.forEach(palace => {
    ...
    const brightStars = mainStars.filter(star => 
      star.brightness && brightnessLevels[star.brightness] >= 4
    )
    ...
    const foundSpecialStars = mainStars.filter(star => 
      [leadershipStars, intellectualStars, executiveStars, creativityStars, communicationStars, harmonyStars]
...
const potentialDevelopment = computed(() => {
  ...
  if (!props.chartData?.palaces) return potentials
  ...
  props.chartData.palaces.forEach(palace => {
    ...
    // 檢查可改善的星曜配置
    const improvableStars = mainStars.filter(star => 
      star.brightness && brightnessLevels[star.brightness] <= 2 && brightnessLevels[star.brightness] >= 0
    )
    ...
    const beneficStars = [...auxiliaryStars, ...palace.stars].filter(star => 
      ['左輔', '右弼', '文昌', '文曲', '天魁', '天鉞', '祿存', '化祿', '化權', '化科'].includes(star.name)
    )
...
const keyFocus = computed(() => {
  ...
  if (!props.chartData?.palaces) return focus
...
const analyzeChartStructure = () => {
  if (!props.chartData?.palaces) return { patterns: [], starTypes: [], energyBalance: 'unknown' }
  ...
  if (leadershipCount >= 1) results.starTypes.push('領導型')
  if (intellectualCount >= 1) results.starTypes.push('智謀型')
  if (executiveCount >= 1) results.starTypes.push('執行型')
  
  return results
}

// 生成核心洞察
const generateCoreInsights = (analysis: any, energyDist: Record<string, number>) => {
...
const getTalentDescription = (palaceName: string, keyStars: string[]): string => {
...
const getCorePotential = (palaceName: string, potentialTypes: string[]): string => {
...
const getPotentialDescription = (palaceName: string, potentialTypes: string[]): string => {
...
const getDevelopmentMethod = (palaceName: string, potentialTypes: string[]): string => {
...
const getStructuralAdvice = (): string => {
  if (!props.chartData?.palaces) return ''
...
const externalTraits = computed(() => {
  ...
  if (!props.chartData?.palaces) return ['外表沉穩內斂，給人可靠的印象', '在社交場合中表現得體，善於察言觀色']
...
const internalTraits = computed(() => {
  ...
  if (!props.chartData?.palaces) return ['內心渴望安全感，重視情感的穩定', '具有同理心，能夠理解他人的感受']
...
const keyPatterns = computed(() => {
  ...
  if (!props.chartData?.palaces) return patterns
...
.star-config, .development-method {
  margin-top: 8px;
  font-size: 0.85rem;
  color: #6c757d;
}

.star-label, .method-label {
  font-weight: 500;
  color: #495057;
}
...
.dominant-element, .element-balance {
  display: flex;
  justify-content: space-between;
  align-items: center;
  padding: 8px 0;
...
.dominant-element:last-child, .element-balance:last-child {
  border-bottom: none;
}
...
@media (max-width: 768px) {
  .elements-container, .patterns-container {
    padding: 0 16px;
  }
  
  .elements-chart, .elements-analysis, .pattern-card {
    padding: 16px;
  }
