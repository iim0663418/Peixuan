...
class ErrorHandler {
   ...
   * 分析錯誤類型和等級
   */
  private analyzeError(error: Error): { category: ErrorCategory; level: ErrorLevel } {
   ...
   * 生成用戶友善的錯誤消息
   */
  private generateUserMessage(category: ErrorCategory, level: ErrorLevel): string {
    switch (category) {
   ...
   * 顯示用戶通知
   */
  private showUserNotification(category: ErrorCategory, level: ErrorLevel): void {
   ...
   * 處理未捕獲的Promise拒絕
   */
  public handleUnhandledRejection(event: PromiseRejectionEvent, userId?: string): void {
