...
export const initializeChartData = (): boolean => {
  try {
    ...
    const purpleStarBirthInfo = getFromStorage(
      STORAGE_KEYS.PURPLE_STAR_BIRTH_INFO,
    );
    const integratedBirthInfo = getFromStorage(
      STORAGE_KEYS.INTEGRATED_BIRTH_INFO,
    );
...
export const validateStorageData = (): boolean => {
  try {
    ...
    if (
      baziChart &&
      unifiedData.charts &&
      unifiedData.charts.bazi !== undefined
    ) {
      try {
        const baziConsistency = crossValidateChartData(
          baziChart,
          unifiedData.charts.bazi,
          'bazi',
        );
      ...
      baziChart &&
      (!unifiedData.charts || unifiedData.charts.bazi === undefined)
    ) {
      // 統一資料中缺少八字命盤
      hasInconsistency = true;
      ...
      !baziChart &&
      unifiedData.charts &&
      validateChart(unifiedData.charts.bazi)
    ) {
      // 獨立存儲中缺少八字命盤
      hasInconsistency = true;
    ...
    if (
      purpleStarChart &&
      unifiedData.charts &&
      unifiedData.charts.purpleStar !== undefined
    ) {
      try {
        const purpleStarConsistency = crossValidateChartData(
          purpleStarChart,
          unifiedData.charts.purpleStar,
          'purpleStar',
        );
      ...
      purpleStarChart &&
      (!unifiedData.charts || unifiedData.charts.purpleStar === undefined)
    ) {
      // 統一資料中缺少紫微斗數命盤
      hasInconsistency = true;
      ...
      !purpleStarChart &&
      unifiedData.charts &&
      validateChart(unifiedData.charts.purpleStar)
    ) {
      // 獨立存儲中缺少紫微斗數命盤
      hasInconsistency = true;
      ...
      // 修復：添加到獨立存儲
      saveToStorage(
        STORAGE_KEYS.PURPLE_STAR_CHART,
        unifiedData.charts.purpleStar,
      );
    }
    ...
    if (
      integratedChart &&
      unifiedData.charts &&
      unifiedData.charts.integrated !== undefined
    ) {
      try {
        const integratedConsistency = crossValidateChartData(
          integratedChart,
          unifiedData.charts.integrated,
          'integrated',
        );
      ...
      integratedChart &&
      (!unifiedData.charts || unifiedData.charts.integrated === undefined)
    ) {
      // 統一資料中缺少整合分析
      hasInconsistency = true;
      ...
      !integratedChart &&
      unifiedData.charts &&
      validateChart(unifiedData.charts.integrated)
    ) {
      // 獨立存儲中缺少整合分析
      hasInconsistency = true;
      ...
      // 修復：添加到獨立存儲
      saveToStorage(
        STORAGE_KEYS.INTEGRATED_ANALYSIS,
        unifiedData.charts.integrated,
      );
    }
 ...
 * 交叉驗證命盤資料
 */
export const crossValidateChartData = (
  chart1: any,
  chart2: any,
  chartType: string,
): { isConsistent: boolean; missingFields: string[] } => {
  const result = {
    isConsistent: true,
  ...
  try {
    ...
    if (chartType in keyFields) {
      for (const field of keyFields[chartType]) {
        ...
        const chart1HasField =
          chart1 && typeof chart1 === 'object' && field in chart1;
        const chart2HasField =
          chart2 && typeof chart2 === 'object' && field in chart2;
