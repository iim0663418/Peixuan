<template>
  <div class="user-input-form">
    ...
    <div v-else-if="!isCalendarLibFullyAvailable && activeCalendarLib && activeCalendarLib.startsWith('lunarJS')" class="form-group error-banner" style="background-color: #fffbe6; color: #8a6d3b; border-left-color: #ffe58f;">
    ...
    <form @submit.prevent="handleSubmit">
      <div class="form-group">
        <label>日期類型: </label>
        <select v-model="formState.calendarType" :disabled="activeCalendarLib === null">
          <option value="solar">國曆</option>
          <option value="lunar" :disabled="activeCalendarLib === null">農曆</option>
        </select>
      </div>
      <div class="form-group">
        <label for="year">出生年份:</label>
        <input type="number" id="year" v-model.number="formState.year" />
      </div>
      <div class="form-group">
        <label for="month">出生月份:</label>
        <input type="number" id="month" v-model.number="formState.month" min="1" max="12" />
      </div>
      <div class="form-group" v-if="formState.calendarType === 'lunar' && activeCalendarLib !== null">
        <label for="isLeapMonth">是否閏月:</label>
        <input type="checkbox" id="isLeapMonth" v-model="formState.isLeapMonth" />
      </div>
      <div class="form-group">
        <label for="day">出生日期:</label>
        <input type="number" id="day" v-model.number="formState.day" min="1" :max="maxDay" />
      </div>
      <div class="form-group">
        <label for="hour">出生小時 (0-23):</label>
        <input type="number" id="hour" v-model.number="formState.hour" min="0" max="23" />
      </div>
      <div class="form-group">
        <label for="minute">出生分鐘 (0-59):</label>
        <input type="number" id="minute" v-model.number="formState.minute" min="0" max="59" />
      </div>
      <div class="form-group" v-if="chineseHourDisplay && isCalendarLibFullyAvailable">
        <label>對應時辰:</label>
      ...
      <div class="form-actions">
        ...
        <button type="button" @click="resetForm" class="button-secondary" :disabled="isLoading">重置表單</button>
      </div>
    ...
    <div class="section-card">
      ...
      <div class="form-group">
        ...
        <input type="number" id="yearlyFate" v-model.number="yearlyFateInput" placeholder="例如 2024" :disabled="!parsedBaziResult || isLoading" />
      </div>
      <YearlyFateTimeline
    ...
    <div v-if="parsedYearlyInteractionResult && yearlyFateInput" class="yearly-interaction-result section-card">
      <h4>流年互動分析 ({{ yearlyFateInput }}年)</h4>
      <div v-if="parsedYearlyInteractionResult.yearStemInteractions.length > 0">
        <h5>天干互動:</h5>
        <ul><li v-for="(interaction, index) in parsedYearlyInteractionResult.yearStemInteractions" :key="'stem-' + index">{{ interaction.description }}</li></ul>
      </div>
      <div v-if="parsedYearlyInteractionResult.yearBranchInteractions.length > 0">
        <h5>地支互動:</h5>
        <ul><li v-for="(interaction, index) in parsedYearlyInteractionResult.yearBranchInteractions" :key="'branch-' + index">流年地支與{{ interaction.pillarName }}地支{{ interaction.pillarBranch }}: {{ interaction.relations.relationDescription || '無特殊關係' }}</li></ul>
      </div>
      <div v-if="parsedYearlyInteractionResult.significantInteractions.length > 0">
        <h5>重要提示:</h5>
        <ul><li v-for="(highlight, index) in parsedYearlyInteractionResult.significantInteractions" :key="'highlight-' + index">{{ highlight }}</li></ul>
      </div>
      <p v-if="!parsedYearlyInteractionResult.yearStemInteractions.length && !parsedYearlyInteractionResult.yearBranchInteractions.length && !parsedYearlyInteractionResult.significantInteractions.length">此流年與命盤無顯著天干或地支刑沖合害關係。</p>
    </div>

    <div v-if="conversionDisplayResult" class="conversion-result section-card"><h4>轉換結果</h4><p>{{ conversionDisplayResult }}</p></div>
    <div v-if="parsedBaziResult && activeCalendarLib && activeCalendarLib.startsWith('lunarJS')" class="bazi-result section-card">
      <h4>八字排盤結果 (文字版)</h4>
      <div class="pillars-grid">
        <div class="pillar-card"><h5>年柱</h5><p>{{ parsedBaziResult.yearPillar.stem }} ({{ parsedBaziResult.yearPillar.stemElement }})</p><p>{{ parsedBaziResult.yearPillar.branch }} ({{ parsedBaziResult.yearPillar.branchElement }})</p></div>
        <div class="pillar-card"><h5>月柱</h5><p>{{ parsedBaziResult.monthPillar.stem }} ({{ parsedBaziResult.monthPillar.stemElement }})</p><p>{{ parsedBaziResult.monthPillar.branch }} ({{ parsedBaziResult.monthPillar.branchElement }})</p></div>
        <div class="pillar-card"><h5>日柱</h5><p>{{ parsedBaziResult.dayPillar.stem }} ({{ parsedBaziResult.dayPillar.stemElement }})</p><p>{{ parsedBaziResult.dayPillar.branch }} ({{ parsedBaziResult.dayPillar.branchElement }})</p></div>
        <div class="pillar-card"><h5>時柱</h5><p>{{ parsedBaziResult.hourPillar.stem }} ({{ parsedBaziResult.hourPillar.stemElement }})</p><p>{{ parsedBaziResult.hourPillar.branch }} ({{ parsedBaziResult.hourPillar.branchElement }})</p></div>
      </div>
    </div>
    <BaziChart v-if="parsedBaziResult && activeCalendarLib && activeCalendarLib.startsWith('lunarJS')" :bazi="parsedBaziResult" :ten-gods="parsedTenGodsResult" />
    <div v-if="parsedTenGodsResult && activeCalendarLib && activeCalendarLib.startsWith('lunarJS')" class="ten-gods-result section-card">
      <h4>主要十神分析</h4>
      <div class="ten-gods-grid">
        <div class="god-item"><strong>年柱天干:</strong> {{ parsedTenGodsResult.yearStemGod }}</div>
        <div class="god-item"><strong>月柱天干:</strong> {{ parsedTenGodsResult.monthStemGod }}</div>
        <div class="god-item"><strong>日柱天干:</strong> {{ parsedTenGodsResult.dayStemGod }}</div>
        <div class="god-item"><strong>時柱天干:</strong> {{ parsedTenGodsResult.hourStemGod }}</div>
      </div>
    </div>
    <div v-if="parsedElementsDistributionResult && activeCalendarLib && activeCalendarLib.startsWith('lunarJS')" class="elements-distribution-result section-card">
      <h4>五行能量分佈 (文字版)</h4>
      <ul><li v-for="(count, element) in parsedElementsDistributionResult" :key="element">{{ element }}: {{ count }}</li></ul>
    </div>
    <ElementsChart v-if="parsedElementsDistributionResult && activeCalendarLib && activeCalendarLib.startsWith('lunarJS')" :distribution="parsedElementsDistributionResult" />
    <div v-if="parsedStartLuckResult && activeCalendarLib && activeCalendarLib.startsWith('lunarJS')" class="start-luck-result section-card">
      <h4>起運資訊</h4>
      <p v-if="parsedStartLuckResult.error">{{ parsedStartLuckResult.error }}</p>
      <div v-else><p>年齡: {{ parsedStartLuckResult.age }}</p><p>年份: {{ parsedStartLuckResult.year }}</p></div>
    </div>
  </div>
</template>
...
import { 
  BaziCalculator, type BaziResult, type UserInputData as BaziInputData, 
  TenGodsCalculator, type TenGodsPillars,
  FiveElementsAnalyzer, type ElementsDistribution,
  FortuneCycleCalculator, type StartLuckInfo,
  type HeavenlyStem, type EarthlyBranch, Solar, Lunar
} from '../utils/baziCalc'; 
import { YearlyInteractionAnalyzer, type YearlyInteractionResult } from '../utils/yearlyInteractionUtils';
...
const parsedStartLuckResult = ref<StartLuckInfo | { error: string } | null>(null);
...
interface FormState {
  calendarType: 'solar' | 'lunar'; year: number | null; month: number | null; day: number | null;
  hour: number | null; minute: number | null; gender: 'male' | 'female';
  timezone: string; isLeapMonth: boolean; 
}
...
function checkCalendarLibrary() {
  if (typeof window.Lunar === 'function' || (typeof window.Lunar === 'object' && window.Lunar !== null && typeof window.Lunar.fromDate === 'function')) {
    activeCalendarLib.value = 'lunarJS_latest_core_only'; isCalendarLibFullyAvailable.value = false; 
    try {
      ...
      if (lunarDate && typeof lunarDate.getEightChar === 'function') {
        ...
        if (eightChar && typeof eightChar.getYun === 'function') {
          activeCalendarLib.value = 'lunarJS_latest_full'; isCalendarLibFullyAvailable.value = true; calendarLibLoadErrorText.value = ''; 
        } else { calendarLibLoadErrorText.value = '日曆庫核心功能可用，但起運等高級功能受限 (getYun方法缺失或EightChar結構不符)。'; }
      } else { calendarLibLoadErrorText.value = '日曆庫核心功能可用，但八字結構 (getEightChar) 缺失，導致起運等高級功能受限。'; }
    } catch (e) {
      console.error('Error during API check for lunar-javascript@latest:', e);
      calendarLibLoadErrorText.value = '檢查日曆庫高級功能時出錯。'; isCalendarLibFullyAvailable.value = false;
    }
    clearTimers(); updateResultsDisplayBasedOnLibStatus();
  }
}
...
function handleCalendarCheckTimeout() {
  ...
  if (typeof window.Lunar === 'object' && typeof window.Solar === 'object' && typeof window.LunarMonth === 'object') {
    ...
    activeCalendarLib.value = null; isCalendarLibFullyAvailable.value = false;
...
function updateResultsDisplayBasedOnLibStatus() {
  if (activeCalendarLib.value === null) {
    ...
    parsedBaziResult.value = null; parsedTenGodsResult.value = null; parsedElementsDistributionResult.value = null; 
...
onMounted(() => {
  ...
  if (typeof window.Lunar === 'object' && typeof window.Solar === 'object') { checkCalendarLibrary(); } 
    ...
    calendarCheckInterval.value = setInterval(checkCalendarLibrary, CHECK_INTERVAL) as any as number;
    calendarCheckTimeout.value = setTimeout(handleCalendarCheckTimeout, MAX_CHECK_DURATION) as any as number;
...
const clearTimers = () => {
  if (calendarCheckInterval.value !== null) clearInterval(calendarCheckInterval.value);
  if (calendarCheckTimeout.value !== null) clearTimeout(calendarCheckTimeout.value);
  calendarCheckInterval.value = null; calendarCheckTimeout.value = null;
};
...
const maxDay = computed(() => {
  if (!activeCalendarLib.value || !formState.year || !formState.month) return 31;
  if (formState.calendarType === 'solar') { return new Date(formState.year, formState.month, 0).getDate(); } 
  else { 
...
watch(() => formState.calendarType, () => { formState.day = null; if (formState.calendarType === 'solar') formState.isLeapMonth = false; });
  ...
  if (bazi && yearInput && yearInput >= 1900 && yearInput <= 2200) {
    if (typeof window.Solar !== 'undefined' && typeof window.Lunar !== 'undefined') {
      try {
        ...
        } else { parsedYearlyInteractionResult.value = null; }
      } catch (e) { parsedYearlyInteractionResult.value = null; }
    } else { parsedYearlyInteractionResult.value = null; }
  } else { parsedYearlyInteractionResult.value = null; }
}, { deep: true });

watch(
  () => [formState.calendarType, formState.year, formState.month, formState.day, formState.hour, formState.minute, formState.isLeapMonth, formState.gender],
  async ([calType, year, month, day, hour, minute, isLeap, gender]) => {
    if (!(year && month && day && hour !== null && minute !== null)) {
      ...
      parsedBaziResult.value = null; parsedTenGodsResult.value = null; parsedElementsDistributionResult.value = null; parsedStartLuckResult.value = null;
      return;
    }
    if (!activeCalendarLib.value) {
      conversionDisplayResult.value = calendarLibLoadErrorText.value;
      parsedBaziResult.value = null; parsedTenGodsResult.value = null; parsedElementsDistributionResult.value = null; parsedStartLuckResult.value = { error: calendarLibLoadErrorText.value };
      return;
    }
    isLoading.value = true;
    conversionDisplayResult.value = ''; parsedBaziResult.value = null; parsedTenGodsResult.value = null; 
    ...
    try {
      let resultText = ''; let solarForBazi: Date | null = null; let currentLunarDateForLuck: any = null;
      if (activeCalendarLib.value && activeCalendarLib.value.startsWith('lunarJS')) {
        if (typeof window.Lunar === 'undefined' || typeof window.Solar === 'undefined') {
            conversionDisplayResult.value = "lunar-javascript 核心組件未定義。"; isLoading.value = false; return;
        }
        if (calType === 'solar') {
          const solar = window.Solar.fromYmdHms(year as number, month as number, day as number, hour as number, minute as number, 0);
          ...
          solarForBazi = new Date(year as number, (month as number) - 1, day as number, hour as number, minute as number, 0);
        } else {
          const currentMonthForLunar = isLeap ? -(month as number) : (month as number);
          currentLunarDateForLuck = window.Lunar.fromYmdHms(year as number, currentMonthForLunar, day as number, hour as number, minute as number, 0);
      ...
      if (activeCalendarLib.value && activeCalendarLib.value.startsWith('lunarJS') && solarForBazi) {
        ...
        if (calculatedBazi && currentLunarDateForLuck) {
          parsedBaziResult.value = calculatedBazi; baziDisplayResult.value = JSON.stringify(calculatedBazi, null, 2);
          const calculatedTenGods = TenGodsCalculator.getMainStemTenGods(calculatedBazi);
          parsedTenGodsResult.value = calculatedTenGods; tenGodsDisplayResult.value = JSON.stringify(calculatedTenGods, null, 2);
          const calculatedElements = FiveElementsAnalyzer.calculateElementsDistribution(calculatedBazi);
          parsedElementsDistributionResult.value = calculatedElements; elementsDistributionDisplayResult.value = JSON.stringify(calculatedElements, null, 2);
          if (isCalendarLibFullyAvailable.value) { 
            ...
            parsedStartLuckResult.value = { error: calendarLibLoadErrorText.value }; startLuckDisplayResult.value = JSON.stringify({ error: calendarLibLoadErrorText.value }, null, 2);
          }
        } else {
          parsedBaziResult.value = null; baziDisplayResult.value = '八字排盤失敗 (lunarJS)。';
          parsedStartLuckResult.value = null; startLuckDisplayResult.value = '';
        }
      }
    } catch (e) {
...
const chineseHourDisplay = computed(() => {
  if (!isCalendarLibFullyAvailable.value || formState.hour === null) return '';
  ...
  if (formState.hour === 0 || formState.hour === 23) index = 0; 
  else index = Math.floor((formState.hour + 1) / 2);
...
watch(selectedYearFromTimeline, (newVal) => { if (newVal) { console.log('Year selected from timeline:', newVal); }});

const handleSubmit = async () => {
  if (formState.year === null || formState.year < 1900 || formState.year > 2100) { alert('請輸入有效的年份 (1900-2100)。'); return; }
  if (formState.month === null || formState.month < 1 || formState.month > 12) { alert('請輸入有效的月份 (1-12)。'); return; }
  if (formState.day === null || formState.day < 1 || formState.day > maxDay.value) { alert(`請輸入有效的日期 (1-${maxDay.value})。`); return; }
  if (formState.hour === null || formState.hour < 0 || formState.hour > 23) { alert('請輸入有效的小時 (0-23)。'); return; }
  ...
  if (!conversionDisplayResult.value || conversionDisplayResult.value.includes('失敗') || conversionDisplayResult.value.includes('無效')) { 
    alert('請檢查輸入的日期資訊是否正確並已成功轉換。'); isLoading.value = false; return; 
  }
  if (activeCalendarLib.value && activeCalendarLib.value.startsWith('lunarJS')) { 
    if (!baziDisplayResult.value || baziDisplayResult.value.includes('失敗')) { alert('八字排盤失敗，請檢查輸入。'); isLoading.value = false; return; }
  ...
  try {
    ...
    if (activeCalendarLib.value && activeCalendarLib.value.startsWith('lunarJS')) {
      ...
      } else { dataToEmit.startLuck = { error: startLuckDisplayResult.value || '起運功能受限或計算失敗' }; }
    }
    ...
    if (activeCalendarLib.value && activeCalendarLib.value.startsWith('lunarJS') && isCalendarLibFullyAvailable.value) {
       if (baziDisplayResult.value && tenGodsDisplayResult.value && elementsDistributionDisplayResult.value && startLuckDisplayResult.value &&
        ...
        const recordToSave: Omit<QueryRecord, 'id' | 'timestamp'> = {
          userId: userId.value as string, inputData: { ...formState }, 
  ...
  } catch (e) { console.error("解析結果或保存查詢紀錄到 localStorage 失敗:", e); alert("處理結果或保存紀錄時發生錯誤。"); }
  finally { isLoading.value = false; }
};

const resetForm = () => {
  formState.calendarType = 'solar'; formState.year = null; formState.month = null; formState.day = null;
  formState.hour = null; formState.minute = null; formState.gender = 'male';
  formState.timezone = Intl.DateTimeFormat().resolvedOptions().timeZone; formState.isLeapMonth = false;
  conversionDisplayResult.value = ''; parsedBaziResult.value = null; parsedTenGodsResult.value = null;
  parsedElementsDistributionResult.value = null; parsedStartLuckResult.value = null;
...
<style scoped>
.user-input-form {
  max-width: 600px; margin: 20px auto; padding: 25px; border-radius: 8px;
...
.form-group label { display: block; margin-bottom: 6px; font-weight: 500; color: #454545; }
...
.form-group input[type="number"]:focus, .form-group input[type="text"]:focus, .form-group select:focus {
  border-color: #42b983; outline: none; box-shadow: 0 0 0 3px rgba(66, 185, 131, 0.2); 
}
.form-actions { display: flex; justify-content: flex-start; gap: 12px; margin-top: 25px; }
button {
  padding: 10px 18px; border: none; border-radius: 4px; font-size: 1rem;
  font-weight: 500; cursor: pointer; transition: background-color 0.2s, transform 0.1s; 
}
button:active { transform: translateY(1px); }
button[type="submit"] { background-color: #42b983; color: white; }
button[type="submit"]:hover { background-color: #36a374; }
button[type="submit"]:disabled { background-color: #a5d6a7; cursor: not-allowed; }
.button-secondary { background-color: #6c757d; color: white; }
.button-secondary:hover { background-color: #5a6268; }
.button-secondary:disabled { background-color: #adb5bd; cursor: not-allowed; }
small { display: block; margin-top: 5px; font-size: 0.875em; color: #666; }
.error-banner {
  background-color: #fdecea; color: #c0392b; padding: 12px 18px; 
  border-left: 5px solid #e74c3c; border-radius: 4px; margin-bottom: 20px; 
}
.error-banner p { margin: 0; font-weight: 500; }
.loading-indicator { text-align: center; margin-top: 15px; color: #42b983; font-weight: 500; }
.conversion-result {
  margin-top: 25px; padding: 18px; border: 1px solid #e0e0e0;
  border-radius: 4px; background-color: #f9f9f9;
}
.conversion-result h4, .section-card h4 {
  margin-top: 0; margin-bottom: 10px; color: #333; font-weight: 600;
  border-bottom: 1px solid #eee; padding-bottom: 8px;
}
.conversion-result p, .section-card p, .section-card li {
  margin-bottom: 0; white-space: pre-wrap; word-break: break-all;
  color: #555; line-height: 1.6; 
}
.section-card ul { list-style-type: none; padding-left: 0; }
.section-card li { padding: 4px 0; }
.pillars-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(120px, 1fr)); gap: 15px; margin-top: 10px; }
.pillar-card {
...
.pillar-card h5 { margin-top: 0; margin-bottom: 10px; font-size: 1.1em; color: #42b983; }
.pillar-card p { margin: 5px 0; font-size: 0.95em; }
.ten-gods-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 10px; margin-top: 10px; }
.god-item { padding: 8px 0; font-size: 0.95em; }
.section-card {
  margin-top: 25px; padding: 18px; border: 1px solid #e0e0e0;
  border-radius: 6px; background-color: #f9f9f9;
}
@media (max-width: 768px) {
  .user-input-form { margin: 15px; padding: 20px; box-shadow: 0 2px 8px rgba(0, 0, 0, 0.08); }
