...
export class FrontendValidator {
     ...
     * 驗證出生時間
     */
    static validateBirthTime(birthTime) {
        const errors = [];
        if (!birthTime) {
            errors.push('請選擇出生時間');
            return { isValid: false, errors };
     ...
     * 驗證地理位置（紫微斗數專用）
     */
    static validateLocationForPurpleStar(location) {
        const errors = [];
        if (!location) {
     ...
     * 驗證紫微斗數表單
     */
    static validatePurpleStarForm(data) {
        const allErrors = [];
        // 驗證基本資料
        const dateResult = this.validateBirthDate(data.birthDate || '');
        const timeResult = this.validateBirthTime(data.birthTime || '');
        ...
        allErrors.push(...genderResult.errors);
        // 驗證地理位置（紫微斗數必需）
        const locationResult = this.validateLocationForPurpleStar(data.location || {});
     ...
     * 驗證農曆轉換是否成功
     */
    static validateLunarConversion(lunarInfo) {
        const errors = [];
        if (!lunarInfo) {
            errors.push('農曆轉換失敗');
            return { isValid: false, errors };
        ...
        // 檢查必要的農曆資料
        const requiredFields = ['year', 'month', 'day', 'yearGan', 'yearZhi', 'monthGan', 'monthZhi', 'dayGan', 'dayZhi', 'timeGan', 'timeZhi'];
...
export const DEFAULT_CITY_COORDINATES = {
    ...
    kaohsiung: { longitude: 120.3014, latitude: 22.6273, timezone: 'Asia/Taipei', name: '高雄' },
    taichung: { longitude: 120.6736, latitude: 24.1477, timezone: 'Asia/Taipei', name: '台中' },
    shanghai: { longitude: 121.4737, latitude: 31.2304, timezone: 'Asia/Shanghai', name: '上海' },
    beijing: { longitude: 116.4074, latitude: 39.9042, timezone: 'Asia/Shanghai', name: '北京' },
    hongkong: { longitude: 114.1694, latitude: 22.3193, timezone: 'Asia/Hong_Kong', name: '香港' },
    tokyo: { longitude: 139.6917, latitude: 35.6895, timezone: 'Asia/Tokyo', name: '東京' },
    seoul: { longitude: 126.9780, latitude: 37.5665, timezone: 'Asia/Seoul', name: '首爾' },
    singapore: { longitude: 103.8198, latitude: 1.3521, timezone: 'Asia/Singapore', name: '新加坡' },
    london: { longitude: -0.1276, latitude: 51.5074, timezone: 'Europe/London', name: '倫敦' },
    newyork: { longitude: -74.0060, latitude: 40.7128, timezone: 'America/New_York', name: '紐約' },
    losangeles: { longitude: -118.2437, latitude: 34.0522, timezone: 'America/Los_Angeles', name: '洛杉磯' }
};
/**
 * 格式化錯誤訊息
 */
...
//# sourceMappingURL=frontendValidation.js.map