...
import { ReadingLevel, READING_LEVEL_CONFIGS, RESPONSIVE_CONFIGS, DEFAULT_USER_PREFERENCES, DEFAULT_TRANSITION_CONFIG, ResponsiveBreakpoint } from '@/types/layeredReading';
// 響應式斷點檢測
const isMobile = useMediaQuery('(max-width: 767px)');
const isTablet = useMediaQuery('(min-width: 768px) and (max-width: 1023px)');
const isDesktop = useMediaQuery('(min-width: 1024px)');
// 全局狀態
const globalReadingState = reactive({
...
export function useLayeredReading() {
    ...
    // 計算當前層級配置
    const currentLevelConfig = computed(() => READING_LEVEL_CONFIGS[globalReadingState.currentLevel]);
    // 計算是否可以升級到更高層級
     ...
     * 切換閱覽層級
     */
    const switchToLevel = async (targetLevel, animated = true) => {
    ...
    const autoSelectOptimalLevel = () => {
        ...
        const fallbackLevel = availableLevels.value.find(level => availableLevels.value.indexOf(level) >= availableLevels.value.indexOf(defaultLevel)) || availableLevels.value[0];
     ...
     * 動畫處理
     */
    const animateTransition = async (fromLevel, toLevel) => {
...
export function useDataAdapter() {
     ...
     */
    const adaptIntegratedAnalysis = (originalData) => {
        const now = new Date();
        ...
        return {
            ...
            layers: {
                summary: {
                    coreTraits: createLayeredContent(extractCoreTraits(analysisData), ReadingLevel.SUMMARY, '核心特質'),
                    currentFortune: createLayeredContent(extractCurrentFortune(analysisData), ReadingLevel.SUMMARY, '近期運勢')
                },
                compact: {
                    personalityHighlights: createLayeredContent(extractPersonalityHighlights(analysisData), ReadingLevel.COMPACT, '性格亮點'),
                    fortuneTrends: createLayeredContent(extractFortuneTrends(analysisData), ReadingLevel.COMPACT, '運勢趨勢'),
                    quickAdvice: createLayeredContent(extractQuickAdvice(analysisData), ReadingLevel.COMPACT, '快速建議')
                },
                standard: {
                    personalityAnalysis: createLayeredContent(analysisData.consensusFindings || [], ReadingLevel.STANDARD, '性格分析'),
                    lifeStages: createLayeredContent(extractLifeStages(analysisData), ReadingLevel.STANDARD, '人生階段'),
                    relationships: createLayeredContent(extractRelationships(analysisData), ReadingLevel.STANDARD, '人際關係'),
                    careerGuidance: createLayeredContent(extractCareerGuidance(analysisData), ReadingLevel.STANDARD, '事業指導'),
                    healthWellness: createLayeredContent(extractHealthWellness(analysisData), ReadingLevel.STANDARD, '健康養生'),
                    recommendations: createLayeredContent(analysisData.recommendations || [], ReadingLevel.STANDARD, '綜合建議')
                },
                deep: {
                    elementalAnalysis: createLayeredContent(extractElementalAnalysis(analysisData), ReadingLevel.DEEP_ANALYSIS, '五行深度分析'),
                    cosmicInfluences: createLayeredContent(extractCosmicInfluences(analysisData), ReadingLevel.DEEP_ANALYSIS, '星曜影響'),
                    transformationCycles: createLayeredContent(extractTransformationCycles(analysisData), ReadingLevel.DEEP_ANALYSIS, '四化週期'),
                    detailedForecasts: createLayeredContent(extractDetailedForecasts(analysisData), ReadingLevel.DEEP_ANALYSIS, '詳細預測'),
                    spiritualGuidance: createLayeredContent(extractSpiritualGuidance(analysisData), ReadingLevel.DEEP_ANALYSIS, '心靈指導'),
                    actionPlans: createLayeredContent(extractActionPlans(analysisData), ReadingLevel.DEEP_ANALYSIS, '行動計劃')
                }
            }
        };
    };
    // 輔助函數
    const createLayeredContent = (items, level, title) => ({
        level,
        visible: true,
    ...
    const extractCurrentFortune = (data) => {
        return ['整體運勢平穩向上，適合穩紮穩打'];
    };
    const extractPersonalityHighlights = (data) => {
    ...
    const extractFortuneTrends = (data) => {
        return [
            '近期運勢：穩中有升，宜把握機會',
            '中期展望：事業發展順遂，財運漸佳',
            '長遠趨勢：人生漸入佳境，前景光明'
    ...
    const extractCareerGuidance = (data) => {
        return [
            '適合穩定發展的行業',
            '重視團隊合作與人際關係',
            '可考慮管理或顧問類職位'
    ...
    const extractHealthWellness = (data) => {
        return [
            '注意腸胃保養，飲食清淡',
            '多運動強身，增強體質',
            '保持心情愉快，避免過度焦慮'
    ...
    const extractElementalAnalysis = (data) => {
        return [
            '五行中土元素較旺，性格穩重',
            '金元素適中，理性務實',
            '水元素偏弱，需增強靈活性'
    ...
    const extractCosmicInfluences = (data) => {
        return [
            '命宮主星影響性格基調',
            '財帛宮配置影響財運走向',
            '遷移宮顯示外出發展機會'
    ...
    const extractTransformationCycles = (data) => {
        return [
            '當前大運利於事業發展',
            '流年四化帶來新機遇',
            '十年一輪，把握轉運時機'
    ...
    const extractDetailedForecasts = (data) => {
        return [
            '未來三個月事業運勢上升',
            '半年內財運有所改善',
            '一年後可能有重要轉變'
    ...
    const extractSpiritualGuidance = (data) => {
        return [
            '保持內心平靜，多行善事',
            '培養感恩的心，珍惜當下',
            '持續學習成長，完善自我'
...
// 直接導出適配器函數供外部使用
export const adaptIntegratedAnalysisToLayered = (originalData) => {
    const { adaptIntegratedAnalysis } = useDataAdapter();
    return adaptIntegratedAnalysis(originalData);
};
//# sourceMappingURL=useLayeredReading.js.map